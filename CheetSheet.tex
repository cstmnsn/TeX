...
\noindent\textbf{包除原理}：事象 $A$，$B$ について，$A$ または $B$ が起きる（和事象）確率は一般に次の式で求められる。
\[
...
\noindent\textbf{事象の独立}：事象 $A$ と $B$ について，次の式が成り立つこと。
\[
P(A \cap B) = P(A)P(B)
\]

\noindent\textbf{条件付き確率}：$A$ が起きたもとで $B$ が起きる確率を次の式の右辺で定義する。
\[
...
\noindent\textbf{条件付き独立}：事象 $A,B,C$ について，$C$ を与えたもとで $A$ と $B$ が条件付き独立であるとは，
\[
P(A \cap B \mid C) = P(A\mid C)\,P(B\mid C)
\]

\noindent\textbf{確率変数}：事象に対して実数を対応させる関数。離散型・連続型がある。

\noindent\textbf{確率関数}：離散型確率変数 $X$ のとりうるすべての値について，
\[
p(x) \ge 0, \qquad \sum_x p(x) = 1
\]

\noindent\textbf{確率密度関数}：連続型確率変数 $X$ の定義域全体について，
\[
f(x) \ge 0, \qquad \int_{-\infty}^{\infty} f(x)\,dx = 1
\]

\noindent\textbf{期待値（平均）}：確率変数がとりうる値の起きやすさを加味した平均。離散型：
\[
...
\noindent\textbf{分散}：確率変数がとりうる値の期待値からのばらつき具合の平均。$E[X]=\mu$ として，離散型：
\[
...
\noindent\textbf{累積分布関数}：$F(x) = P(X\le x)$。非減少関数で，分布関数ともいう。$X$ が連続型で $F$ が微分可能，$f$ が連続ならば $F'(x)=f(x)$。

\noindent\textbf{同時累積分布関数}：$F(x,y) = P(X\le x, Y\le y)$。

\noindent\textbf{同時確率関数}：離散型 $X,Y$ について $p(x,y)=P(X=x,Y=y)$。\textbf{周辺確率関数}：
\[
p_X(x) = \sum_y p(x,y)
\]

\noindent\textbf{条件付き確率関数}：$p_X(x)\ne 0$ のとき，
\[
p_{Y\mid X}(y\mid x) = \frac{p(x,y)}{p_X(x)}
\]

\noindent\textbf{同時確率密度関数}：$F(x,y)$ が2回連続微分可能なとき，
\[
...
\noindent\textbf{条件付き確率密度関数}：$f_X(x)\ne 0$ のとき，
\[
f_{Y\mid X}(y\mid x) = \frac{f(x,y)}{f_X(x)}
\]

\noindent\textbf{条件付き期待値}：
\[
...
\noindent\textbf{確率変数の独立}：
\[
f(x,y) = f_X(x)f_Y(y) \quad(\text{または }p(x,y)=p_X(x)p_Y(y))
\]

\noindent\textbf{条件付き独立}：$Z=z$ を与えたとき，
\[
f(x,y\mid z) = f_{X\mid Z}(x\mid z)\,f_{Y\mid Z}(y\mid z)
\]

\noindent\textbf{確率母関数}：離散型 $X$ の確率関数を $p(x)$ とし，1に十分近いすべての $s$ に対して次のべき級数が収束するとき，
\[
...
\noindent\textbf{モーメント（積率）母関数}：0に十分近いすべての $t$ に対し次の期待値が存在するとき，
\[
M_X(t) = E[e^{tX}]
\]
...
\noindent\textbf{分位点}：$F$ が増加関数のとき，$F(x_{0.25})=0.25$ を満たす $x_{0.25}$ を第1四分位数，$x_{0.5}$（$F(x_{0.5})=0.5$）を中央値，$x_{0.75}$ を第3四分位数，$x_{0.75}-x_{0.25}$ を四分位範囲という。

\noindent\textbf{最頻値（モード）}：$f(x)$（または $p(x)$）を最大にする $x$。

\noindent\textbf{変動係数}：非負値の確率変数 $X$ について，
\[
CV = \frac{\sigma}{\mu}
\]
標準偏差 $\sigma$ を平均 $\mu$ で割ることで単位が相殺され，無次元量になる。そのため，単位や平均の水準が異なる複数のデータ間でも，相対的なばらつきの大きさを直接比較できる（標準偏差同士の単純比較ではこれができない）。

\noindent\textbf{歪度}：$E[X]=\mu,\ V[X]=\sigma^2$ として，
\[
...
\noindent\textbf{共分散}：$E[X]=\mu_X,\ E[Y]=\mu_Y$ として，
\[
...
\noindent\textbf{偏相関係数}：$Z$ が $X,Y$ の両方に相関があるとき，直接的な相関がなくても擬似相関が生じる。$Z$ の影響を除いた $X,Y$ の偏相関係数は，
\[
\rho_{XY\cdot Z} = \frac{\rho_{XY} - \rho_{XZ}\rho_{YZ}}{\sqrt{(1-\rho_{XZ}^2)(1-\rho_{YZ}^2)}}
\]

\noindent\textbf{加重平均}：重み $w_1,\dots,w_n$ に対し，
\[
\bar{x}_w = \frac{\sum_{i=1}^n w_i x_i}{\sum_{i=1}^n w_i}
\]
$w_1=\cdots=w_n=1/n$ のとき算術平均に一致。

\noindent\textbf{幾何平均}：正のデータについて，
\[
...
\noindent\textbf{確率変数ベクトル・平均ベクトル}：$\bm{x}=(x_1,\dots,x_p)^T$ について，$E[\bm{x}]=(E[x_1],\dots,E[x_p])^T$。$A$ を $r\times p$ 定数行列，$\bm{b}$ を $r$ 次元定数ベクトルとして，
\[
E[A\bm{x}+\bm{b}] = A E[\bm{x}] + \bm{b}
\]

\noindent\textbf{分散共分散行列}：
\[
\Sigma = \big(\Cov[x_i,x_j]\big)_{i,j=1,\dots,p}
\]
半正定値対称行列。

\noindent\textbf{標本分散共分散行列}：サイズ $n$ の標本に対し，$(i,j)$成分を
\[
s_{ij} = \frac{1}{n-1}\sum_{k=1}^n (x_{ki}-\bar{x}_i)(x_{kj}-\bar{x}_j)
\]
とする $p$ 次正方行列。

\noindent\textbf{相関行列・標本相関行列}：$(i,j)$成分を $\rho[x_i,x_j]$（または標本相関係数）とする半正定値対称行列。

\section{変数変換}

\noindent\textbf{$Y=g(X)$ の確率密度関数}：$X,Y$ を連続型，$y=g(x)$ が単調増加または単調減少で逆関数 $x=g^{-1}(y)$ が微分可能なとき，
\[
...
\noindent\textbf{$X+Y$ の確率密度関数}：$X,Y$ が独立な連続型確率変数のとき，$Z=X+Y$ の確率密度関数は畳み込みで与えられる。
\[
f_Z(z) = \int_{-\infty}^{\infty} f_X(z-t)\,f_Y(t)\,dt
\]
（変数変換 $(x,y)\to(z,t)=(x+y,y)$ のヤコビアンは1）

\noindent\textbf{Box-Cox変換}：非負のデータ $x$ を正規分布に近づける変換。
\[
x^{(\lambda)} =
\begin{cases}
...
\noindent\textbf{離散一様分布}：$\{1,\dots,k\}$ 上で等確率。
\[
...
\noindent\textbf{ベルヌーイ分布 $\mathrm{Bin}(1,p)$}：
\[
p(x)=p^x(1-p)^{1-x}\ (x=0,1),\qquad E[X]=p,\qquad V[X]=p(1-p),\qquad G_X(s)=1-p+ps
\]

\noindent\textbf{二項分布 $\mathrm{Bin}(n,p)$}：
\[
...
\noindent\textbf{ポアソン分布 $Po(\lambda)$}：
\[
...
\noindent\textbf{幾何分布 $Geo(p)$}（初成功までの失敗回数 $X$）：
\[
...
\noindent\textbf{負の二項分布 $NB(r,p)$}（$r$回目の成功までの失敗回数）：
\[
...
\noindent\textbf{多項分布 $M(n;p_1,\dots,p_k)$}：
\[
...
\noindent\textbf{正規分布 $N(\mu,\sigma^2)$}：
\[
...
\noindent\textbf{ガンマ分布 $Ga(a,b)$}：（$a$：形状母数，$b$：尺度母数）
\[
...
\noindent\textbf{ベータ分布 $Be(a,b)$}：
\[
f(x)=\frac{1}{B(a,b)}x^{a-1}(1-x)^{b-1}\ (0<x<1),\qquad
E[X]=\frac{a}{a+b},\qquad V[X]=\frac{ab}{(a+b)^2(a+b+1)}
\]

\noindent\textbf{対数正規分布 $\Lambda(\mu,\sigma^2)$}：$Y\sim N(\mu,\sigma^2)$，$X=e^Y$。
\[
...
\noindent\textbf{多変量正規分布 $N(\bm\mu,\Sigma)$}：
\[
...
\noindent\textbf{混合正規分布}：例えば $N(0,1)$ の密度 $\varphi_1$，$N(4,1)$ の密度 $\varphi_2$ として，
\[
f(x) = 0.3\varphi_1(x)+0.7\varphi_2(x)
\]
一般に，$\pi_1+\cdots+\pi_k=1\ (\pi_i>0)$，正規密度 $\varphi_1,\dots,\varphi_k$ として $f=\sum_i \pi_i\varphi_i$。

\noindent\textbf{EMアルゴリズム}（2成分混合正規分布 $f=\pi\varphi_1+(1-\pi)\varphi_2$，$\bm\theta=(\mu_1,\sigma_1^2,\mu_2,\sigma_2^2,\pi)$ の推定）：
...
\noindent\textbf{カイ2乗分布 $\chi^2(n)$}：$Ga(n/2,2)$。
...
非心カイ2乗分布 $\chi^2(n,\lambda)$：$\lambda=\mu_1^2+\cdots+\mu_n^2>0$ として，$\sum_i(Z_i+\mu_i)^2\sim\chi^2(n,\lambda)$。
再生性（一般化）：$X_i\sim\chi^2(n_i,\lambda_i)$独立なら $\sum X_i\sim\chi^2\!\left(\sum n_i,\sum\lambda_i\right)$。

\noindent\textbf{$t$分布 $t(n)$}：$Z\sim N(0,1),\,Y\sim\chi^2(n)$独立として，
\[
...
\noindent\textbf{$F$分布 $F(n_1,n_2)$}：$X_1\sim\chi^2(n_1),X_2\sim\chi^2(n_2)$独立として，
\[
...
\noindent\textbf{概収束}（例：大数の強法則）：
\[
P\big(\lim_{n\to\infty} X_n = Y\big) = 1
\]

\noindent\textbf{平均二乗収束}（例：大数の弱法則，確率収束の例でもある）：
\[
\lim_{n\to\infty} E\big[(X_n-Y)^2\big] = 0
\]

\noindent\textbf{確率収束}（$X_n\to_p Y$）：任意の $\varepsilon>0$ に対し，
\[
\lim_{n\to\infty} P(|X_n-Y|>\varepsilon) = 0
\]

\noindent\textbf{分布収束（法則収束）}（$F_n\to_d G$）：$G$のすべての連続点$x$で，
\[
\lim_{n\to\infty} F_n(x) = G(x)
\]

\noindent\textbf{大数の弱法則}：$X_1,\dots,X_n$が平均$\mu$，分散有限の同一分布に独立に従うとき，標本平均は$\mu$に平均二乗収束する。

\noindent\textbf{中心極限定理}：$X_1,\dots,X_n$が平均$\mu$，分散$\sigma^2$の同一分布に独立に従うとき，
\[
\frac{\bar X_n - \mu}{\sigma/\sqrt n} \ \to_d\ N(0,1)
\]

\noindent\textbf{連続修正}：$X\sim\mathrm{Bin}(n,p)$，$Y\sim N[np,np(1-p)]$として，$P(a\le X\le b)$を$P(a-0.5\le Y\le b+0.5)$で近似する。

\noindent\textbf{連続写像定理}：$X_n\to_d X$で$h$が連続なら $h(X_n)\to_d h(X)$。

\noindent\textbf{スルツキーの補題}：$U_n\to_d U,\ V_n\to_p a$ のとき，
\[
U_n+V_n \to_d U+a,\qquad U_nV_n \to_d aU
\]

\noindent\textbf{デルタ法}：$\sqrt n(U_n-\mu)\to_d N(0,\sigma^2)$ のとき，導関数が連続で $h'(\mu)\ne0$ を満たす $h$ に対し，
\[
...
\noindent\textbf{順序統計量}：$X_{(1)} \le X_{(2)} \le \cdots \le X_{(n)}$。

\noindent\textbf{モーメント}：$E[X^k]$ を原点まわりの$k$次モーメント，$E[(X-\mu)^k]$を平均まわりの$k$次モーメントという。

\noindent\textbf{モーメント法}：大数の法則に基づく母数推定法。$k$次までのモーメントが $\bm\theta=(\theta_1,\dots,\theta_k)$の関数として表されているとき，$j$次モーメントを $\frac1n\sum X_i^j$ で置き換えて $\theta_1,\dots,\theta_k$ について解く。
...
\noindent\textbf{最尤法}：観測値 $x_1,\dots,x_n$ に対する\textbf{尤度関数}
\[
...
\noindent\textbf{漸近正規性}：適当な正則条件のもとで，$n\to\infty$のとき，
\[
...
\noindent\textbf{バイアス・バリアンス分解}：
\[
E\big[(\hat\theta-\theta)^2\big] = V[\hat\theta] + \big(E[\hat\theta]-\theta\big)^2
\]
（平均2乗誤差 ＝ 分散＋バイアス$^2$）

\noindent\textbf{不偏推定量}：バイアスが0の推定量。標本分散はバイアスがあるが，不偏分散
\[
U^2 = \frac{1}{n-1}\sum_{i=1}^n(X_i-\bar X)^2
\]
は母分散の不偏推定量。

\noindent\textbf{ジャックナイフ推定量}：バイアスを補正する方法。①標本全体からの推定量 $\hat\theta$。②$X_i$を除いた推定量 $\hat\theta_{(i)}$の平均 $\overline{\hat\theta_{(\cdot)}}$。③新推定量 $= n\hat\theta-(n-1)\overline{\hat\theta_{(\cdot)}}$。

\noindent\textbf{UMVUE}：不偏推定量の中で分散最小のもの（一様最小分散不偏推定量）。

\noindent\textbf{クラメール・ラオの不等式}：不偏推定量 $\hat\theta$ に対し，
\[
V[\hat\theta] \ge \frac{1}{n J_1(\theta)}
\]

\noindent\textbf{漸近有効推定量}：$n\to\infty$で分散がクラメール・ラオの下限を達成する一致推定量。\\
\textbf{有効推定量}：不偏推定量の中でクラメール・ラオの下限を達成するもの（＝UMVUE）。

\noindent\textbf{十分統計量}：$T(\bm X)=t$ を与えたときの $\bm X=\bm x$ の条件付き確率が $\theta$ に依存しないこと。同値な条件（\textbf{フィッシャー・ネイマンの分解定理}）：
\[
...
\noindent\textbf{分散の比の区間推定}：2つの正規母集団から求めた不偏分散を $U_A^2,U_B^2$（独立），母分散を$\sigma_A^2,\sigma_B^2$として，信頼度95\%の信頼区間：
\[
...
\noindent\textbf{多項分布の差の信頼区間}：$(X_1,\dots,X_k)\sim M(n;p_1,\dots,p_k)$として，$p_1-p_2$の信頼度95\%の信頼区間：
\[
...
\noindent\textbf{検出力とサンプルサイズ}：母比率 $p$，有意水準2.5\%の片側検定，帰無仮説 $p=p_0$，対立仮説 $p=p_1\ (p_1>p_0)$。棄却限界値は
\[
...
\noindent\textbf{生産者危険}：帰無仮説が正しいのに誤って棄却する誤り（\textbf{第一種の過誤}）。不良品率 $p_0$，サンプルサイズ $n$，合格基準値 $c$ として，
\[
P(\text{不良品数}>c) = \sum_{x=c+1}^{n}\binom{n}{x}p_0^x(1-p_0)^{n-x}
\]

\noindent\textbf{消費者危険}：対立仮説が正しいのに誤って帰無仮説を受容する誤り（\textbf{第二種の過誤}）。不良品率 $p_1$として，
\[
...
\noindent\textbf{ポアソン分布に関する検定}：$\lambda$が大きいとき $Po(\lambda)\approx N(\lambda,\lambda)$。$\lambda=\lambda_0$を帰無仮説として，
\[
Z = \frac{X-\lambda_0}{\sqrt{\lambda_0}} \ \sim\ N(0,1) \quad(\text{近似})
\]

\noindent\textbf{適合度検定}：$(X_1,\dots,X_k)\sim M(n,p_1,\dots,p_k)$，帰無仮説 $p_i=p_{i0}$。期待度数 $np_{i0}$。
\[
...
\noindent\textbf{尤度比検定}：$\bm\theta=(\bm\theta_1,\bm\theta_2)$のうち $\bm\theta_1$ を定数に固定する帰無仮説に対し，
\[
...
\noindent\textbf{符号検定}：$Z=X-m_0$（1標本）または $Z=X-Y$（対応2標本）。手順：① $Z$の符号を「＋」「－」に分け，0は除く（サイズを改めて$n$）。② ＋の個数 $T$ が帰無仮説下で $\mathrm{Bin}(n,1/2)$ に従うことを用いて$P$値を計算（$n$が大なら$N(n/2,n/4)$で近似）。

\noindent\textbf{符号付き順位検定}：母集団分布の対称性を仮定。① $Z$の符号を分ける。② $|Z|$の小さい順に順位付け。③ ＋の順位合計 $T_+$ を検定統計量とし，$n$が大でタイなしなら
\[
T_+ \ \sim\ N\!\left(\frac{n(n+1)}{4},\ \frac{n(n+1)(2n+1)}{24}\right)\quad(\text{近似})
\]

\noindent\textbf{ウィルコクソンの順位和検定}：$X_1,\dots,X_m\sim F$，$Y_1,\dots,Y_n\sim G$独立，$F,G$は分布形が同じと仮定。① 全データを合わせて順位付け。② 一方の群の順位和 $W$ が，$m,n$大でタイなしのとき
\[
W \ \sim\ N\!\left(\frac{m(m+n+1)}{2},\ \frac{mn(m+n+1)}{12}\right)\quad(\text{近似})
\]

\noindent\textbf{並べ替え検定}：標本を固定したもとで統計量 $T$（平均値・中央値など）の分布をつくり，観測値以上に極端な値となる確率（$P$値）を求める。

\noindent\textbf{クラスカル・ウォリス検定}：$k$群，サイズ $n_1,\dots,n_k$（$N=\sum n_i$），順位和 $R_1,\dots,R_k$，順位平均 $\bar R_1,\dots,\bar R_k$。タイなしなら，
\[
H = \frac{12}{N(N+1)}\sum_{i=1}^k n_i\left(\bar R_i-\frac{N+1}{2}\right)^2 \ \to_d\ \chi^2(k-1)
\]

\noindent\textbf{順位相関係数}：タイなしのサイズ$n$の2変量順位データ $(x_1,y_1),\dots,(x_n,y_n)$。
...
\textbf{ケンドールの順位相関係数}：正の組数 $P$，負の組数 $N$（$(x_i-x_j)(y_i-y_j)$の符号）として，
\[
\tau = \frac{P-N}{n(n-1)/2}
\]
...
\noindent\textbf{状態確率ベクトル}：$\bm\pi_n=(p_n(1),\dots,p_n(N))$。$\bm\pi_0$を\textbf{初期分布}という。\\
\textbf{推移確率行列}：$\bm\pi_{n+1}=\bm\pi_n Q$ を満たす $N$次正方行列 $Q$。\\
\textbf{定常分布}：$\bm\pi=\bm\pi Q$ を満たす $\bm\pi$。ある条件下でただ1つ存在する。

\noindent\textbf{パラメータの推定}：観測データ $X_0=x_0,\dots,X_n=x_n$ を用いた最尤推定は，
\[
...
\textbf{確率過程}：$X_1,X_2,\dots$ を離散時間の確率過程，$t\ge 0$ 実数全体をとる $X=(X_t)$ を連続時間の確率過程という。

\noindent\textbf{ブラウン運動}：$B=(B_t)$ が次を満たす。
\begin{enumerate}[label=\arabic*.]
\item $B_0=0$，パスは連続。
...
\noindent\textbf{ポアソン過程}：強度 $\lambda$ のポアソン過程 $N=(N_t)$ は次を満たす。
\begin{enumerate}[label=\arabic*.]
\item $N_0=0$。
\item 独立増分性。
...
\noindent\textbf{複合ポアソン過程}：$N=(N_t)$がポアソン過程，$(U_k)$が独立同分布で$N$と独立として，
\[
...
\noindent\textbf{最小2乗法}：$X^TX$ が正則なら，誤差平方和 $\bm\varepsilon^T\bm\varepsilon$ を最小にする $\bm\beta$（\textbf{最小2乗推定量}）は，
\[
...
\noindent\textbf{自由度調整済み決定係数}：
\[
\bar R^2 = 1-\frac{n-1}{n-d-1}(1-R^2)
\]

\noindent\textbf{回帰係数の推定と検定}：$\bm\varepsilon\sim N(\bm 0,\sigma^2 I)$ なら $\hat{\bm\beta}\sim N(\bm\beta,\sigma^2(X^TX)^{-1})$。$\beta_i=0$ の検定：
\[
...
\noindent\textbf{一般化最小2乗法}：誤差項が不均一分散・相関ありの場合，適切な変換を組み込んだ推定量がBLUE。

\noindent\textbf{L$_2$正則化（リッジ回帰）}：
\[
...
\noindent\textbf{L$_1$正則化（Lasso回帰）}：
\[
...
\noindent\textbf{Elastic Net}：L$_1$とL$_2$を混ぜたもの。変数選択と係数の縮小を同時に行う。\\
\textbf{Fused lasso}：隣接する回帰係数の差にL$_1$罰則を課す（時間的に滑らかな変化を表現）。

\section{回帰診断法}

\noindent\textbf{残差プロット}：縦軸に残差，横軸に予測値。誤差項の独立性・分散一定性の仮定が崩れていないか確認。\\
\textbf{正規Q-Qプロット}：縦軸に標準化残差の経験分位点，横軸に標準正規分布の理論分位点。正規性が成り立てば傾き1の直線に沿う。

\noindent\textbf{てこ比}：予測値ベクトル $\hat{\bm y}=X(X^TX)^{-1}X^T\bm y$。行列 $H=X(X^TX)^{-1}X^T$ の対角成分 $h_{ii}$。

\noindent\textbf{Cookの距離}：$i$番目のデータを除いたときの予測値の変化を用いる。$\hat y_j,\hat y_{j(-i)}$（$i$番目を除いたときの予測値）として，
\[
...
\noindent\textbf{一般化線形モデル}：1対1の関数 $g$（\textbf{リンク関数}）を用いて，
\[
...
\noindent\textbf{ロジスティック回帰モデル}：$\bm\beta^T\bm x=\beta_0+\beta_1x_1+\cdots+\beta_kx_k$として，
\[
...
\noindent\textbf{プロビット回帰モデル}：標準正規分布の累積分布関数 $\Phi$ を用いて，
\[
...
\noindent\textbf{トービットモデル}：潜在変数 $Y^*=\beta_0+\beta_1X_1+\beta_2X_2+\varepsilon$ を考え，$Y^*\le0$なら$Y=0$，$Y^*>0$なら$Y=Y^*$。$\varepsilon_i\sim N(0,\sigma^2)$独立とすると，$y_i^*\mid x_i \sim N(\beta_0+\beta_1x_{i1}+\beta_2x_{i2},\sigma^2)$。$y_i>0$ の観測に対する密度：
\[
...
\noindent\textbf{生存時間}：基準時点からイベント発生までの時間。\\
...
\noindent\textbf{カプラン・マイヤー法}：時点$t$までのイベント発生時点を用いて，生存率を
\[
\hat S(t) = \prod_{t_i\le t}\left(1-\frac{d_i}{n_i}\right)
\]
（$d_i$：時点$t_i$での発生数，$n_i$：直前のリスク集合数）で推定し，階段状にプロットしたものを\textbf{カプラン・マイヤー曲線}という。

\noindent\textbf{Cox比例ハザードモデル}：共変量 $x_1,\dots,x_n$ として，
\[
h(t\mid \bm x) = h_0(t)\exp(\bm\beta^T\bm x)
\]
$h_0(t)$は\textbf{基準ハザード}（分布形を仮定しない）。\textbf{比例ハザード性}（ハザード比が時間で不変）の確認が必要。

\noindent\textbf{ニューラルネットワークモデル}：入力 $x_1,\dots,x_d$，重み $w_1,\dots,w_d$，バイアス $b$，活性化関数 $f$ として，1ユニットの出力は
\[
...
\noindent\textbf{フィッシャーの3原則}：①無作為化（処理をランダムに割当），②反復（同じ処理を複数回），③局所管理（ブロック内で均一な条件）。①②のみ満たすものを\textbf{完全無作為化法}，①〜③すべて満たすものを\textbf{乱塊法}という。

\noindent\textbf{1元配置完全無作為化法}：水準 $A_1,\dots,A_a$ の母平均 $\mu_1,\dots,\mu_a$，効果 $\alpha_1,\dots,\alpha_a$として，
\[
...
\noindent\textbf{1元配置乱塊法}：ブロック効果 $\gamma_j$ として，
\[
...
\noindent\textbf{2元配置完全無作為化法}：因子$A,B$，水準効果$\alpha_i,\beta_j$，交互作用 $(\alpha\beta)_{ij}$ として，
\[
...
\noindent\textbf{2元配置乱塊法}：ブロック効果 $\gamma_k$ を追加した構造モデル。平方和は $S_T=S_A+S_B+S_R+S_{A\times B}+S_E$ に分解。

\noindent\textbf{直交表}：一部の交互作用のみ取り上げて実験回数を減らす\textbf{一部実施要因計画}に用いる。例：$L_8(2^7)$（2水準，7列，8実験）。列の割り付けにより交互作用効果が他の主効果と区別できなくなることを\textbf{交絡}という。

\section{標本調査法}

調査単位と抽出単位が一致し，各標本の抽出確率が等しい抽出法を\textbf{単純無作為抽出}という。以下，非復元抽出を考える。

\noindent\textbf{標本平均の期待値と分散}：大きさ$N$の母集団（母平均$\mu$，母分散$\sigma^2$）から大きさ$n$を非復元単純無作為抽出：
\[
...
\noindent\textbf{層化抽出法}：基準を決めていくつかの層に分けた後，各層から無作為抽出する方法。層内均質・層間異質に層化すると精度が上がる。

\noindent\textbf{標本配分法}：層の大きさ $N_1,\dots,N_k$（$\sum N_j=N$），標本の大きさ $n_1,\dots,n_k$（$\sum n_j=n$）。
...
\textbf{ネイマン配分法}（分散最小）：$j$番目の層の母標準偏差 $\sigma_j$ として，
\[
...
\noindent\textbf{寄与率}：$\lambda_i/\sum_j\lambda_j$。\textbf{累積寄与率}：大きい方から合計。\\
...
\noindent\textbf{自己符号化器}：入力 $\bm x$ を低次元表現に符号化 $\bm y=f(W^T\bm x+\bm b)$ し，復号化器で $\hat{\bm x}=g(W'^T\bm y+\bm b')$ のように元の入力に近いデータを再構築するニューラルネットワーク。

\section{判別分析}

\noindent\textbf{フィッシャーの線形判別分析}：2変数 $(x_1,x_2)$，$Y=w_1x_1+w_2x_2=\bm w^T\bm x$。群間変動 $\bm w^TS_B\bm w$ を群内変動 $\bm w^TS_W\bm w$ でわった比を最大化：
\[
...
\noindent\textbf{マハラノビス距離による判別}：標本平均 $\bar{\bm x}$，標本分散共分散行列 $S$ として，
\[
...
\noindent\textbf{2次判別分析}：群1,2の標本分散共分散行列 $S_1,S_2$ として，
\[
...
\noindent\textbf{正準判別分析}：$g$個の群を低次元へ射影。射影軸 $y=\bm w^T\bm x$ の係数ベクトル $\bm w$ は，群間変動対群内変動比の最大化を通して $S_W^{-1}S_B$ の最大固有値に対する固有ベクトルとして得られる。ここで，
\[
...
\noindent\textbf{ハードマージンSVM}：$y_i(\bm w^T\bm x_i+b)\ge1$のもとで $\tfrac12\|\bm w\|^2$ を最小化。解：
\[
\bm w = \sum_i \alpha_i y_i \bm x_i
\]
サポートベクター $\bm x_+,\bm x_-$ を用い，マージンを最大化するように $\bm w,b$ を決める。

\noindent\textbf{ソフトマージンSVM}：スラック変数 $\xi_i\ge0$，$y_i(\bm w^T\bm x_i+b)\ge1-\xi_i$のもとで，
\[
\tfrac12\|\bm w\|^2 + \lambda\sum_i \xi_i \ \to\ \min
\]

\noindent\textbf{混同行列と評価指標}：
\[
...
\textbf{ユークリッド距離}：$\bm x=(x_1,\dots,x_p),\bm y=(y_1,\dots,y_p)$ に対し，
\[
d(\bm x,\bm y) = \sqrt{\sum_{k=1}^p (x_k-y_k)^2}
\]

\noindent\textbf{階層的クラスタリング}：距離の近いクラスターを順次融合。融合過程を表す図を\textbf{デンドログラム}という。クラスター間距離の定義：
\begin{itemize}
...
\noindent\textbf{寄与率}（$f_1$の寄与率，上の例）：
\[
\frac{\beta_{11}^2+\beta_{21}^2+\beta_{31}^2}{3}
\]

\noindent\textbf{因子軸の回転}：直交回転の代表例が\textbf{バリマックス回転}，斜交回転の代表例が\textbf{プロマックス回転}。

\noindent\textbf{操作変数法}：パス図に対応する\textbf{構造方程式}を，例えば $Z=aX+bY+u,\ Y=cX+v$ とする。$E[X]=E[Y]=E[Z]=0,\ V[X]=V[Y]=V[Z]=1$，$u$は$X,Y$と，$v$は$X$と無相関を仮定。構造方程式に操作変数をかけて期待値をとると，
\[
...
\noindent\textbf{グラフィカルモデル}：$(X_1,\dots,X_d)$が$d$変量正規分布のとき，変数を頂点に，条件付き独立性を辺の有無で表現。相関行列の逆行列の$(1,2)$成分が0なら，$X_1,X_2$は他の変数を与えたもとで条件付き独立（辺なし）。

\section{その他の多変量解析手法}

\noindent\textbf{多次元尺度構成法}：$n$次距離行列 $D$，$n$次単位行列 $I_n$，全成分1の$n$次正方行列 $J_n$ として，
\begin{enumerate}[label=\arabic*.]
...
\noindent\textbf{正準相関分析}：$\bm x=(x_1,\dots,x_p)^T,\ \bm y=(y_1,\dots,y_q)^T$ について，
\[
...
\noindent\textbf{数量化法}：質的データを数量化して多変量解析する手法（数量化I〜III類）。I類・II類は説明変数と目的変数（\textbf{外的基準}）に二分，III類は変数どうし対等（主成分分析の名義尺度版，外的基準なし）。

\section{時系列解析}

確率変数 $Y_t\ (t=1,2,\dots)$。

\noindent\textbf{弱定常過程（共分散定常過程）}：平均・分散が一定，$h$次自己共分散 $\Cov[Y_t,Y_{t-h}]=\gamma_h$ が時点によらない。$V[Y_t]=\gamma_0$，$h$次自己相関係数 $\gamma_h/\gamma_0$。

\noindent\textbf{ホワイトノイズ}：平均0，分散一定，0でないラグの自己共分散がすべて0。$U_t$：分散$\sigma^2$のホワイトノイズ。

\noindent\textbf{$p$次自己回帰過程 AR($p$)}：
\[
...
\noindent\textbf{$(p,q)$次自己回帰移動平均過程 ARMA($p,q$)}：
\[
...
\noindent\textbf{$(p,1,q)$次自己回帰和分移動平均過程 ARIMA($p,1,q$)}：$\{Y_t\}$は弱定常ではないが，差分系列 $\Delta Y_t=Y_t-Y_{t-1}$ が弱定常かつ反転可能なARMA過程。\textbf{反転可能}：MA部分がAR部分に書き直せること，
\[
1+\theta_1z+\theta_2z^2+\cdots+\theta_qz^q = 0
\]
のすべての解の絶対値が1より大きいことと同値。

\noindent\textbf{ディッキー・フラー検定}：AR(1)モデル $Y_t=\varphi Y_{t-1}+U_t$ について，帰無仮説 $\varphi=1$，対立仮説 $|\varphi|<1$。

\noindent\textbf{状態空間モデル}：状態方程式（時点$t-1$の状態＋誤差）と観測方程式（時点$t$の状態＋観測誤差）を組み合わせたモデル。

\noindent\textbf{ACFとPACF}：$h$次自己相関係数 $\gamma_h/\gamma_0$ を$h$の関数と見たものが\textbf{自己相関関数（ACF）}。$Y_t,Y_{t-h}$間の相関から間の$Y_{t-1},\dots,Y_{t-h+1}$の影響を除いたものが偏自己相関係数で，これを$h$の関数と見たものが\textbf{偏自己相関関数（PACF）}。
\[
\begin{array}{lcc}
...
\noindent\textbf{ダービン・ワトソン検定}：誤差項 $u_t$ にAR(1)モデル $u_t=\rho u_{t-1}+U_t$ を仮定，帰無仮説 $\rho=0$。最小2乗法の残差 $e_t$ を用いて，
\[
DW = \frac{\sum_{t=2}^n (e_t-e_{t-1})^2}{\sum_{t=1}^n e_t^2} \ \approx\ 2(1-\hat\rho)
\]
$DW$が2に近ければ帰無仮説を支持，0や4に近ければ棄却。

\noindent\textbf{スペクトル密度関数}：AR(1)モデル $Y_t=c+\varphi Y_{t-1}+U_t\ (-1<\varphi<1)$：
\[
...
\noindent\textbf{相対リスク}：発症率の比。喫煙群0.15，非喫煙群0.03なら相対リスク $=0.15/0.03=5$。\\
\textbf{前向き研究}／\textbf{後向き研究}，\textbf{コホート研究}／\textbf{ケースコントロール研究}。

\noindent\textbf{オッズ}：$\dfrac{\text{成功確率}}{\text{失敗確率}}$。回復確率0.8なら，オッズ $=0.8/0.2=4$。\\
\textbf{オッズ比}：オッズの比。

\noindent\textbf{標本オッズ・標本オッズ比}：$2\times2$分割表の度数を $a,b,c,d$（行1: $a,b$，行2: $c,d$）とすると，
\[
...
\noindent\textbf{フィッシャーの正確検定}：2元分割表の独立性を，超幾何分布による$P$値の直接計算で検定する。行和・列和を固定した上で，観測結果以上に極端な結果となる確率を超幾何分布の確率関数の和として求める。

\noindent\textbf{独立性の検定}：$a\times b$分割表，全体度数 $n$，$(i,j)$セルの度数 $n_{ij}$，行和 $n_{i\cdot}$，列和 $n_{\cdot j}$ として，
\[
...
\noindent\textbf{クラメールの連関係数}：
\[
V = \sqrt{\frac{\chi^2}{n(k-1)}}
\]
（$k$：分割表の行数・列数のうち大きくない方）。$0\le V\le1$。

\noindent\textbf{逸脱度}：フルモデルの尤度と一部のパラメータを0とおいたモデルの尤度の比を $\lambda$，観測度数 $x_{ij}$，モデルのもとでの期待度数 $m_{ij}$として，
\[
-2\log\lambda = 2\sum_{i,j} x_{ij}\log\frac{x_{ij}}{m_{ij}} \ \to_d\ \chi^2
\]
（自由度＝帰無仮説と対立仮説の自由パラメータ数の差）

\noindent\textbf{対数線形モデル}：3元分割表のフルモデルは，同時確率 $p_{ijk}$ について，
\[
...
\noindent\textbf{無向独立グラフ}：因子を頂点，2因子交互作用を含む頂点間に辺をひいたグラフ。\\
...
\noindent\textbf{打ち切り}：ある値以上（または以下）であることはわかるが，その値自体は得られないタイプの欠測。\\
\textbf{トランケーション}：特定の幅の値しか観測できず，この幅に入っていないデータの個数もわからないタイプの欠測。

\noindent\textbf{欠測メカニズム}：
\begin{itemize}
...
\noindent\textbf{CC(complete case)解析}：欠測が生じた個体を除いた解析。サンプルサイズ減少により精度・検出力が下がり，MCAR以外ではバイアスが生じる。

\noindent\textbf{単一値代入法}：平均値代入，回帰代入，確率的回帰代入，ホットデック法などがある。

\noindent\textbf{多重代入法}：複数組の擬似的な完全データを作成し，独立に推定値を計算して統合する。不確実性を作り出し，単一代入法での誤差の過小評価を補正する。

\noindent\textbf{1変量正規分布における推測}：$X\sim N(\mu,\sigma^2)$，標準正規密度$\varphi$，累積分布関数$\Phi$，$a=(c-\mu)/\sigma$として，$X\ge c$のもとでの条件付き期待値：
\[
...
\noindent\textbf{2変量正規分布における推測（MAR，$Y$のみ欠測）}：観測データ $(x_1,y_1),\dots,(x_m,y_m),x_{m+1},\dots,x_n$（$m<n$）。
\begin{enumerate}[label=\arabic*.]
...
\noindent\textbf{AIC（赤池情報量規準）}：
\[
\mathrm{AIC} = -2\times(\text{最大対数尤度}) + 2\times(\text{推定パラメータ数})
\]

\noindent\textbf{BIC（ベイズ情報量規準）}：
\[
...
\noindent\textbf{交差検証法（クロスバリデーション）}：データを訓練用・テスト用に分け，訓練用で推定，テスト用で予測誤差を検証することを繰り返し，予測誤差最小のモデルを選ぶ。

\noindent\textbf{Leave-one-out法}：
\begin{enumerate}[label=\arabic*.]
...
\noindent\textbf{$k$分割交差検証法}：データを$k$グループに分割し，1グループをテスト用，残り$k-1$グループを訓練用として予測誤差を$k$回求める。$k=n$がLeave-one-out法。

\section{ベイズ法}

パラメータ自体が確率分布に従うと仮定し，データを得る前の\textbf{事前分布}をデータの情報を反映した\textbf{事後分布}に更新する。

\noindent\textbf{共役事前分布}：事前分布と事後分布が同じタイプになる事前分布。
\[
\begin{array}{lll}
...
\noindent\textbf{ベータ二項モデル}：$n$回中$y$回成功のとき，事後分布は $Be(a+y,\,b+n-y)$。\\
\textbf{ガンマポアソンモデル}：事後分布は $Ga\!\left(a+\sum y_i,\ 1/(b+n)\right)$。\\
\textbf{正規-正規モデル}：事後分布は上表の通り。

\noindent\textbf{ベイズ推定量}：事後分布の期待値。$Ga(a,b)$の期待値 $=ab$，$Be(a,b)$の期待値 $=a/(a+b)$。\\
...
\noindent\textbf{MCMC法}：事後分布 $\pi(\bm\theta\mid\bm x)$ からの多数の独立なサンプルを近似的に得る手法。

\noindent\textbf{メトロポリス法}：
\begin{enumerate}[label=\arabic*.]
...
\noindent\textbf{メトロポリス・ヘイスティングス法}：手順2で $\bm\theta'$を非対称な提案分布 $q(\bm\theta\mid\bm\theta^{(t)})$ から発生させ，手順3を，
\[
r = \frac{\pi(\bm\theta'\mid\bm x)\,q(\bm\theta^{(t)}\mid\bm\theta')}{\pi(\bm\theta^{(t)}\mid\bm x)\,q(\bm\theta'\mid\bm\theta^{(t)})}
\]
に置き換えたもの。

\noindent\textbf{ギブスサンプリング}（2変量の場合，条件付き分布 $f(x\mid y),f(y\mid x)$ から抽出可能とする）：
\begin{enumerate}[label=\arabic*.]
...
\noindent\textbf{擬似乱数}：一定の条件下で再現できてしまう，計算機生成の乱数。

\noindent\textbf{逆関数法}：$U\sim U(0,1)$，$X$の累積分布関数 $F_X$として，
\[
X = F_X^{-1}(U)
\]
とすれば$X$の分布に従う乱数が得られる。

\noindent\textbf{採択・棄却法}：目標密度 $f(x)$，扱いやすい密度 $g(x)$，定数$R$ について，すべての$x$で $f(x)\le R\,g(x)$ が成り立つとき：
\begin{enumerate}[label=\arabic*.]
...
\noindent\textbf{モンテカルロ法}（面積推定）：正方形 $(0,1)\times(0,1)$内の図形$A$の面積$S$を，一様乱数$(u,v)$を$n$組発生させ，$A$内に入った個数$X$から $S\approx X/n$ と推定する（$X\sim\mathrm{Bin}(n,S)$，$V[X]=nS(1-S)$）。

\noindent\textbf{モンテカルロ積分}：$X\sim U(0,1)$ のとき，
\[
...
\noindent\textbf{主部の分離}：積分 $I=\int_a^b f(x)\,dx$ に対し，
\begin{enumerate}[label=\arabic*.]
...
\noindent\textbf{負の相関の利用}：
\begin{enumerate}[label=\arabic*.]
\item $g(x) = \dfrac{f(x)+f(a+b-x)}{2}$ とおく。
\item $X_1,\dots,X_m\sim U(a,b)$として，$\hat I = (b-a)\cdot\dfrac1m\sum_i g(X_i)$。
\end{enumerate}

\noindent\textbf{経験分布関数}：分布関数 $F(x)$からの無作為標本 $X_1,\dots,X_n$ に対し，
\[
F_n(x) = \frac{1}{n}\#\{i : X_i\le x\}
\]
$F_n(x)$は$F(x)$に確率収束する。

\noindent\textbf{ジャックナイフ法}：パラメータ$\theta$の推定量 $\hat\theta$。$X_j$を除いたサイズ$n-1$の標本による推定量 $\hat\theta_{(j)}$，
\[
...
\noindent\textbf{ブートストラップ法}：独立同一分布 $F$ に従う $X_1,\dots,X_n$ の実現値 $x_1,\dots,x_n$ から復元抽出でサイズ$n$の標本 $\bm x^{*(b)}$ を $B$回作り，
\[
